...
model_file = "/zhoukangkang/old_tipc-opt/ait_examples/resnet/model16.onnx"
sess = ort.InferenceSession(model_file, sess_options=sess_opt, providers=trt_providers)
input_data = {}
name = "input.1"
real_input = np.zeros([16, 3, 224, 224], dtype=np.float32)
input_data[name] = real_input

X_ortvalue = ort.OrtValue.ortvalue_from_numpy(real_input, 'cuda', 0)
Y_ortvalue = ort.OrtValue.ortvalue_from_shape_and_type([16, 1000], np.float32, 'cuda', 0)  # Change the shape to the actual shape of the output being bound
